...
describe("VOXY final canon lock — V3.10.5 / PR #624", () => {
  ...
  it("05 routes the official homepage render command through a fail-closed canon guard and binds current HEAD automatically", () => {
    ...
    expect(guardedEntrypoint).toContain('spawnSync("git", ["rev-parse", "HEAD"]');
    expect(guardedEntrypoint).toContain("voxy_final_canon_explicit_head_mismatch");
    expect(guardedEntrypoint).toContain(
