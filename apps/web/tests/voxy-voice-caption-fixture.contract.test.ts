...
describe("Voxy voice and caption fixture", () => {
  ...
  it.each([6_450, 12_340])(
    "scales caption timing to the measured audio duration %dms",
    (durationMs) => {
      const plan = buildVoxyVoiceCaptionFixturePlan({
        characterPlan: buildVoxyCharacterMotionFixturePlan("16:9"),
        briefingId: `briefing-audio-${durationMs}`,
        scriptVersion: "script-v3",
        audio: { ...approvedAudio, durationMs },
        reviewStatus: "approved",
        reviewerId: "editor-001",
        reviewedAt: "2026-08-05T20:00:00.000Z",
      });
      ...
      expect(plan.segments.at(-1)?.endMs).toBe(durationMs);
      expect(
        plan.segments.every(
          (segment, index) =>
            index === 0 || segment.startMs === plan.segments[index - 1]?.endMs,
        ),
      ).toBe(true);
      expect(validateVoxyVoiceCaptionFixturePlan(plan)).toEqual({
        ok: true,
  ...
  it("fails closed for overlapping segments and audio mismatch", () => {
    ...
    plan.audio = { ...plan.audio, durationMs: 7_000 };
