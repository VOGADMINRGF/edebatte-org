...
describe("create fast-intake latency contract", () => {
  ...
  it("starts the planner deadline only after durable save", () => {
    ...
    expect(deadlineIndex).toBeGreaterThan(durableSaveIndex);
    ...
    expect(startFlow.match(/fetch\("\/api\/create\/save"/g)).toHaveLength(1);
  ...
  it("keeps the same correlation for timeout retry and reports planner timings", () => {
    ...
    const retryFlow = client.slice(
      client.indexOf("const handleRetryPlanner"),
      client.indexOf("const handleRequestEditorialReview"),
    );

    expect(client).toContain("const plannerCorrelationIdRef = React.useRef<string | null>(null)");
    expect(client).toContain("plannerCorrelationIdRef.current = correlationId");
    expect(client).toContain("plannerCorrelationIdRef.current ??");
    expect(client).toContain("plannerTrace?.requestId ??");
    expect(retryFlow).not.toContain('fetch("/api/create/save"');
    expect(retryFlow).toContain("draftId: savedDraftId");
