...
function getProcedureClaim(dossier: Dossier) {
  return (
    dossier.analyze.claims.find((claim) =>
      `${claim.title ?? ""} ${claim.text}`.toLocaleLowerCase("de-DE").match(/verfahren|verhandlung|ratsbefassung|gesetzgeb/),
    ) ?? dossier.analyze.claims[0] ?? null
  );
}

function getKnownClaims(dossier: Dossier) {
...
function buildContributionHref(dossierId: string, kind?: string) {
...
export default function DossierDecisionCockpit({
  ...
  const knownClaims = useMemo(() => getKnownClaims(dossier), [dossier]);
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]">
        <div className="space-y-5">
          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 sm:p-7">
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              <div>
                <h2 className="text-lg font-semibold text-[rgb(var(--fg))]">Was passiert gerade?</h2>
          ...
          <section id="dossier-decision-vote" className="scroll-mt-28 rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 sm:p-7">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              {votingAvailable ? (
                ...
                >
                  {votePending ? "Speichere…" : voteSummary?.selectedOptionId ? "Stimme aktualisieren" : "Stimme speichern"}
                </button>
              ) : (
              ...
              <Link
                href={buildContributionHref(dossier.meta.id, "contribute")}
              ...
              >
                Fehlende Option oder Argument ergänzen
              </Link>
              {voteError ? <p className="w-full text-sm text-rose-700 dark:text-rose-300">{voteError}</p> : null}
              {voteSummary?.ok && selectedOptionId && voteSummary.selectedOptionId === selectedOptionId ? (
                <p className="w-full text-xs text-[rgb(var(--muted))]">Deine aktuelle Auswahl ist gespeichert.</p>
              ) : null}
            </div>
          </section>
          ...
          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 sm:p-7">
            ...
            <h2 className="mt-1 text-xl font-semibold text-[rgb(var(--fg))]">
              Keine Risikoampel ohne belastbare Grundlage
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[rgb(var(--muted))]">
            ...
            <div className="mt-5 grid gap-3 lg:grid-cols-2">
              {impacts.map((impact) => (
                <article key={`${impact.type}-${impact.description}`} className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--muted))]">Auswirkungsdimension</p>
              ...
              {gaps.map((gap) => (
                <article key={gap.id ?? gap.text} className="rounded-2xl border border-amber-300/70 bg-amber-500/10 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--muted))]">Evidenz- oder Perspektivlücke</p>
                  <h3 className="mt-1 text-sm font-semibold text-[rgb(var(--fg))]">{gap.dimension ?? "Prüfbedarf"}</h3>
          ...
          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 sm:p-7">
            <div className="grid gap-6 lg:grid-cols-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">Belastbar dokumentiert</p>
                <div className="mt-3 space-y-3">
                  {knownClaims.length ? knownClaims.map((claim) => (
                    <article key={claim.id} className="rounded-2xl border border-emerald-300/60 bg-emerald-500/10 p-4">
                      <h3 className="text-sm font-semibold text-[rgb(var(--fg))]">{claim.title ?? claim.text}</h3>
                      <p className="mt-2 text-sm leading-6 text-[rgb(var(--muted))]">{claim.text}</p>
                    </article>
                  )) : (
                    <p className="text-sm text-[rgb(var(--muted))]">Noch keine gestützten Kernaussagen ausgewiesen.</p>
                  )}
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">Offene Prüfungen</p>
                <div className="mt-3 space-y-3">
                  {model.questions.length ? model.questions.slice(0, 5).map((question) => (
                    <article key={question.id} className={`rounded-2xl border p-4 ${question.status === "in_review" ? TONE_CLASSES.review : TONE_CLASSES.open}`}>
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <h3 className="text-sm font-semibold text-[rgb(var(--fg))]">{question.text}</h3>
                        <span className="rounded-full border border-current/20 px-2 py-1 text-[11px] font-semibold text-[rgb(var(--muted))]">
                          {question.statusLabel}
                        </span>
                      </div>
                      {question.responsibility ? (
                        <p className="mt-2 text-xs text-[rgb(var(--muted))]">Zuständig: {question.responsibility}</p>
                      ) : null}
                    </article>
                  )) : (
                    <p className="text-sm text-[rgb(var(--muted))]">Keine offenen Prüfungen dokumentiert.</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <aside className="space-y-5 xl:sticky xl:top-20 xl:self-start">
          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">Stimmung und Parlament</p>
            <div className="mt-4 space-y-4">
              ...
              {parliamentaryPoll ? (
                <div className="rounded-2xl border border-blue-300/60 bg-blue-500/10 p-4">
                  <p className="text-xs text-[rgb(var(--muted))]">Dokumentiertes parlamentarisches Verhalten</p>
                  <h3 className="mt-1 text-sm font-semibold text-[rgb(var(--fg))]">{parliamentaryPoll.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[rgb(var(--fg))]">{parliamentaryPoll.overallResult}</p>
                  {parliamentaryPoll.germanDelegationResult ? (
                    <p className="mt-2 text-xs leading-5 text-[rgb(var(--muted))]">{parliamentaryPoll.germanDelegationResult}</p>
                  ) : null}
                  <a
                ...
                <p className="text-sm text-[rgb(var(--muted))]">Keine eindeutig zuordenbare parlamentarische Abstimmung gefunden.</p>
              )}
            </div>
          </section>

          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">Beitragen</p>
            <h2 className="mt-1 text-lg font-semibold text-[rgb(var(--fg))]">Dossier gemeinsam verbessern</h2>
            <p className="mt-2 text-sm leading-6 text-[rgb(var(--muted))]">
            ...
            <div className="mt-4 grid gap-2">
              {[
                ...
                ["Argument oder Aspekt ergänzen", "claim"],
                ["Offene Frage einreichen", "question"],
                ["Faktenfehler oder Korrektur melden", "check"],
              ].map(([label, kind]) => (
                <Link
                  key={kind}
                  href={buildContributionHref(dossier.meta.id, kind)}
          ...
          <section className="rounded-[2rem] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">Dossier-Reife</p>
