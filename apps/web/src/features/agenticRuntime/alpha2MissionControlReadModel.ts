...
export function summarizeAlpha2MissionRuns(input: {
  ...
  return {
    ...
    statusCounts,
    activeRuns: statusCounts.queued + statusCounts.running + statusCounts.waiting,
    humanInbox: statusCounts.review + statusCounts.human_gate,
    failedRuns: statusCounts.failed,
    scheduledRuns: input.scheduledRuns,
    leasedRuns: input.leasedRuns,
    recentRuns,
  };
}
...
export async function buildAlpha2MissionControlSnapshot(input: {
  ...
  if (Number.isNaN(nowDate.getTime())) throw new Error("alpha2_mission_control_invalid_now");
  ...
  try {
    ...
    const [totalRuns, statusRows, recentRows, scheduledRuns, leasedRuns] = await Promise.all([
