...
export function buildVoxyVoiceCaptionFixturePlan(input: {
  ...
  const sourceTimelineDurationMs = input.characterPlan.scenes.at(-1)?.endMs ?? 0;
  const audioDurationMs = Math.round(input.audio.durationMs);

  if (sourceTimelineDurationMs <= 0) {
    throw new Error("character_timeline_duration_invalid");
  }
  if (audioDurationMs <= 0) {
    throw new Error("audio_duration_invalid");
  }

  const scaleTimelineBoundary = (sourceMs: number): number =>
    Math.round((sourceMs / sourceTimelineDurationMs) * audioDurationMs);

  const segments = input.characterPlan.scenes.flatMap((scene, sceneIndex) => {
    const text = [scene.headline, scene.detail].filter(Boolean).join(" — ");
    const chunks = splitCaptionText(text, maxCharacters);
    const sceneStartMs = sceneIndex === 0 ? 0 : scaleTimelineBoundary(scene.startMs);
    const sceneEndMs =
      sceneIndex === input.characterPlan.scenes.length - 1
        ? audioDurationMs
        : scaleTimelineBoundary(scene.endMs);
    ...
    return chunks.map((chunk, index) => ({
      ...
      startMs:
        index === 0
          ? sceneStartMs
          : sceneStartMs + Math.round((durationMs * index) / chunks.length),
      endMs:
        index === chunks.length - 1
          ? sceneEndMs
          : sceneStartMs + Math.round((durationMs * (index + 1)) / chunks.length),
      text: chunk,
  ...
  return {
    ...
    audio: { ...input.audio, durationMs: audioDurationMs },
    segments,
    safeArea,
