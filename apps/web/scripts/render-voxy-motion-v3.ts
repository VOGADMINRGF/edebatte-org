...
import {
  ...
  VOXY_MOTION_V3_STANDFRAMES,
} from "../src/features/voxyVideo/motionV3";
import {
  buildVoxyMotionV3FrameState,
  renderVoxyMotionV3FrameHtml,
  type VoxyMotionV3EmbeddedAssets,
  type VoxyMotionV3Format,
} from "../src/features/voxyVideo/motionV3Html";
...
async function renderFormat(input: {
  page: Page;
  plan: ReturnType<typeof buildVoxyMotionV3Plan>;
  assets: VoxyMotionV3EmbeddedAssets;
  format: Exclude<VoxyMotionV3Format, "16:9">;
  outputRoot: string;
}) {
  ...
  const frameIndex = Math.floor((19_500 * input.plan.output.fps) / 1_000);
  await setHtml(input.page, renderVoxyMotionV3FrameHtml({ plan: input.plan, assets: input.assets, frameIndex, format: input.format }));
...
async function main(): Promise<void> {
  const exactHeadSha = process.env.VOXY_MOTION_V3_COMMIT_SHA?.trim() ?? "";
  if (!/^[0-9a-f]{40}$/.test(exactHeadSha)) throw new Error("VOXY_MOTION_V3_COMMIT_SHA_must_be_exact_40_char_sha");
  ...
  const evidenceInputs = [
    ".github/workflows/voxy-motion-v3-evidence.yml",
    "apps/web/scripts/generate-voxy-motion-v3-layers.ts",
    "apps/web/scripts/render-voxy-motion-v3.ts",
    "apps/web/src/features/voxyVideo/motionV3.ts",
    "apps/web/src/features/voxyVideo/motionV3Html.ts",
  ...
  const plan = buildVoxyMotionV3Plan(exactHeadSha);
  const errors = validateVoxyMotionV3Plan(plan);
  if (errors.length) throw new Error(`motion_v3_plan_invalid:${errors.join(",")}`);
  ...
  const assets: VoxyMotionV3EmbeddedAssets = {
  ...
  for (let frameIndex = 0; frameIndex < plan.output.frameCount; frameIndex += 1) {
    ...
    await page.setViewportSize({ width: 1920, height: 1080 });
    await setHtml(page, renderVoxyMotionV3FrameHtml({ plan, assets, frameIndex }));
  ...
  const standframes = [];
  for (const standframe of VOXY_MOTION_V3_STANDFRAMES) {
  ...
  for (const frame of [
    ...
    { file: "blink.png", frame: Math.round((1_250 * plan.output.fps) / 1_000) },
    { file: "speaking.png", frame: Math.round((19_000 * plan.output.fps) / 1_000) },
  ...
  await writeFile(path.resolve(motionRoot, plan.output.captionsVttFileName), buildVoxyMotionV3Vtt(), "utf8");
  await writeFile(path.resolve(motionRoot, plan.output.captionsSrtFileName), buildVoxyMotionV3Srt(), "utf8");
  await context.close();
  await browser.close();
  ...
  for (const [label, probe] of [["mp4", mp4Probe], ["webm", webmProbe]] as const) {
    ...
    if (!video || audio || Number(video.width) !== 1920 || Number(video.height) !== 1080 || video.avg_frame_rate !== "24/1" || Math.abs(Number(probe.format.duration) - 25) > 0.05) throw new Error(`${label}_ffprobe_contract_invalid`);
  }
  ...
  console.info(JSON.stringify({ status: "voxy_motion_v3_rendered", exactHeadSha, layerMasterSha, motionArtifact: path.relative(repositoryRoot, motionRoot), layerArtifact: path.relative(repositoryRoot, layerRoot), mp4Sha256: motionManifest.media.mp4.sha256, humanVisualAcceptance: motionManifest.humanVisualAcceptance, productionEligible: false, autoPublish: false }, null, 2));
}
