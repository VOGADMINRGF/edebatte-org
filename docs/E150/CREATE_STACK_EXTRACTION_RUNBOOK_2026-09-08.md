...
Status: Governance- und Extraktionsplan; C4-Parent und C4A-Dekompositions-Parent bleiben `blocked` nach `FAIL_SPLIT_REQUIRED`. C4A1 `CREATE-GUEST-ADOPTION-PREPARATION-SERVER-FOUNDATION-01` ist über PR `#762`, Implementation Head `d503e97848dbfb8726b2a3a930748f9667b984c8`, Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` `done`. Final gilt `NO_PREPARATION_BROWSER_CARRIER_V1`: C3A ist die einzige Browser-Transition-Bindung und es existiert kein Preparation-Locator in Cookie, URL oder Browser-Storage. Ausschließlich C4B ist für einen frischen taskbezogenen Preflight `codex_ready`; seine Implementation ist nicht autorisiert. C4C bleibt `blocked`; C5–C12 bleiben nicht autorisiert. Keine Secret-, Provider-, Deploy- oder Production-Aktivierung ist autorisiert.

Zielbasis: `main@7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` (C4A1 über PR `#762` abgeschlossen; nächster erlaubter Create-Schritt ausschließlich frischer C4B-Preflight)
...
- C3A–C3D sind abgeschlossen. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`; die At-Rest-Foundation und C4A1 sind `done`. C4A1 ist über PR `#762` / Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` abgeschlossen; seine Implementierungsautorisierung ist konsumiert und geschlossen. Ausschließlich C4B ist für frischen `preflight_only` `codex_ready`, `IMPLEMENTATION_AUTHORIZED=false`. C4C sowie C5–C12 und G1–G5 bleiben nicht implementierungsautorisiert.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) und `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) sind gemergt. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`. Die At-Rest-Foundation und C4A1 sind `done`; C4A1 wurde über PR `#762` als Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` abgeschlossen. Der nächste erlaubte Create-Schritt ist ausschließlich ein frischer C4B-Preflight; C4B-Implementation bleibt nicht autorisiert. C4C bleibt bis C4B done `blocked`; C5–C12 bleiben nicht autorisiert.
...
| C4 | Browser Resume, explizite Adoption und Draft Binding | `#682` | C4-Parent und C4A-Dekompositions-Parent `blocked`; At-Rest prerequisite und C4A1 `done`; C4A1 abgeschlossen über PR `#762` / Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d`; ausschließlich C4B ist für einen frischen `preflight_only` autorisiert, Implementation bleibt gesperrt; C4C folgt erst nach C4B done |
...
C3A–C3D sind `done`. Der C4-Parent `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` und C4A `CREATE-GUEST-ADOPTION-PREPARATION-FOUNDATION-01` sind nach `FAIL_SPLIT_REQUIRED` `blocked`, nicht direkt implementierbar und nicht `done`: C3C persistiert keinen Rohtext, C3D bleibt ephemer, und Payload-/Receipt-/Auth-Transition sind getrennte Sicherheitsdomänen. Die historische C4A-Evidence auf `main@0103c90e0364f3da2f86ab56ac4a230fafc74049` lautet `FAIL_BLOCKED` mit `BLOCKER=AT_REST_PROTECTION`; der Blocker ist durch die gemergte At-Rest-Foundation aufgelöst. Der frische Preflight auf `main@232f390df68e9b46a01082a73d9f3259f4a61e38` ergab `FAIL_SPLIT_REQUIRED`. C4A1s `FAIL_BLOCKED` auf `main@82a66db13904b9acd6414e4da044de55145daa51` bleibt historische Evidence; C4A1 ist über PR `#762` / Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` `done`. Ausschließlich C4B ist für einen frischen `preflight_only` `codex_ready`; seine Implementation ist nicht autorisiert. C4C bleibt bis C4B done `blocked`. TTL oder C3C-Safety ersetzen At-Rest-Schutz nicht. Die folgende Source-/Owner-Evidence beschreibt historischen Parent-Scope und ist keine Copy- oder Merge-Freigabe.
...
```text
main (C1 über PR #734, C2 über PR #736, C3A über PR #740, C3B über PR #744, C3C über PR #748 und C3D über PR #751 gemergt; C3D done via main@65a89f67d1a8bb21fac41def3164270cd171b8d5)
            ...
            └─ merge → refreshed main
                 └─ C4 Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                      └─ Temporary Payload At-Rest Protection (done; PR #757; merged)
                           └─ C4A Decomposition Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                                └─ C4A1 Server Preparation Foundation (done; PR #762; merge 7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d)
                                     └─ refreshed main
                                                    └─ C4B fresh preflight only (`codex_ready`; implementation not authorized)
                                                         └─ C4B implementation/review/merge/done → refreshed main
                                                              └─ C4C Guest Intent/Login Transition (blocked until C4B done; separate preflight / authorization)
                                                                   └─ C4C implementation/review/merge/done → refreshed main
...
Dieses Runbook autorisiert keine C4-Implementierung. Die At-Rest-Foundation und C4A1 sind done, ihre Autorisierungen konsumiert; C4A ist als `FAIL_SPLIT_REQUIRED` Dekompositions-Parent blockiert. Ausschließlich C4B ist für einen frischen `preflight_only` registriert; seine Implementation ist nicht autorisiert. C4C, C5–C12 und G1–G5 bleiben nicht autorisiert; keine Secret-, Provider- oder Production-Aktivierung ist autorisiert.
