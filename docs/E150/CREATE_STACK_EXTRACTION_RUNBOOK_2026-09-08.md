...
Status: Governance- und Extraktionsplan; C4-Parent und C4A-Dekompositions-Parent bleiben `blocked` nach `FAIL_SPLIT_REQUIRED`. Der abgeschlossene C4A1-Preflight ist historisch `FAIL_BLOCKED` wegen `DURABLE_FAILED_REPREPARE_REVOCATION`; `DURABLE_REPREPARE_REVOCATION_BARRIER_V1` ist ausschließlich frischer Preflight-Input. C4A1 ist `codex_ready` mit `AUTHORIZATION=preflight_only`; C4B und C4C bleiben `blocked`. Keine Secret-, Provider- oder Production-Aktivierung ist autorisiert.

Zielbasis: `main@82a66db13904b9acd6414e4da044de55145daa51` (C4A1-Blockerresolution und frischer Re-Preflight; frühere SHAs bleiben historische Evidence)
...
- C3A–C3D sind abgeschlossen. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`; die At-Rest-Foundation ist `done`. C4A1s abgeschlossener Preflight ist historisch `FAIL_BLOCKED`; die genehmigte durable Revocation Barrier erlaubt ausschließlich seinen frischen `preflight_only`-Check, `IMPLEMENTATION_AUTHORIZED=false`. C4B, C4C sowie C5–C12 und G1–G5 bleiben nicht autorisiert.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) und `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) sind gemergt. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`. Die At-Rest-Foundation ist über PR `#757` done; ihre frühere Autorisierung ist konsumiert. C4A1s Preflight auf `main@82a66db13904b9acd6414e4da044de55145daa51` ergab `FAIL_BLOCKED`; die genehmigte `DURABLE_REPREPARE_REVOCATION_BARRIER_V1` erlaubt nur einen neuen `preflight_only`-Check. C4B bleibt bis C4A1 done und C4C bis C4B done `blocked`.
...
| C4 | Browser Resume, explizite Adoption und Draft Binding | `#682` | C4-Parent und C4A-Dekompositions-Parent `blocked`; At-Rest prerequisite `done` über PR `#757`; frischer C4A-Preflight `FAIL_SPLIT_REQUIRED`. Ausschließlich C4A1 ist `preflight_only`, ohne Implementierungsautorisierung. C4B bleibt bis C4A1 done, C4C bis C4B done `blocked` |
                 ...
                 └─ C4 Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                      └─ Temporary Payload At-Rest Protection (done; PR #757; merged)
                           └─ C4A Decomposition Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                                └─ C4A1 Server Preparation Foundation (`codex_ready`; `preflight_only`; implementation unauthorized)
                                     └─ fresh C4A1 preflight
                                          └─ C4A1 PASS → separate implementation authorization
                                               └─ C4A1 implementation/review/merge/done → refreshed main
...
Dieses Runbook autorisiert keine C4-Implementierung. Die At-Rest-Foundation ist done und ihre Autorisierung konsumiert; C4A ist als `FAIL_SPLIT_REQUIRED` Dekompositions-Parent blockiert. Ausschließlich C4A1 ist für einen frischen Preflight registriert. C4B, C4C, C5–C12 und G1–G5 bleiben nicht autorisiert; keine Secret-, Provider- oder Production-Aktivierung ist autorisiert.
