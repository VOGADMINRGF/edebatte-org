...
- C3A–C3D sind abgeschlossen. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`; die At-Rest-Foundation ist `done`. C4A1s historischer `FAIL_BLOCKED` bleibt Evidence; sein frischer Re-Preflight autorisiert ausschließlich die separate exakt begrenzte Implementation, `IMPLEMENTATION_AUTHORIZED=true`, `DONE=false`. C4B, C4C sowie C5–C12 und G1–G5 bleiben nicht autorisiert.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) und `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) sind gemergt. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`. Die At-Rest-Foundation ist über PR `#757` done; ihre frühere Autorisierung ist konsumiert. C4A1s Re-Preflight auf `main@a60ce5ee982c99b403966e32aa0ec4d28ee33544` autorisiert seine begrenzte Sechs-Dateien-Implementation. C4B bleibt bis C4A1 done und C4C bis C4B done `blocked`.
...
| C4 | Browser Resume, explizite Adoption und Draft Binding | `#682` | C4-Parent und C4A-Dekompositions-Parent `blocked`; At-Rest prerequisite `done` über PR `#757`; C4A1 Re-Preflight `PASS_FOR_SEPARATE_GOVERNANCE_AUTHORIZATION`. Ausschließlich C4A1 ist für seine exakt sechs Dateien implementierungsautorisiert; C4B bleibt bis C4A1 done, C4C bis C4B done `blocked` |
                 ...
                 └─ C4 Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                      └─ Temporary Payload At-Rest Protection (done; PR #757; merged)
                           └─ C4A Decomposition Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                                └─ C4A1 Server Preparation Foundation (`codex_ready`; implementation authorized; exactly six files)
