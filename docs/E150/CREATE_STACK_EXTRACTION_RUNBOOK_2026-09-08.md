...
Status: Governance- und Extraktionsplan; C1, C2, C3A, C3B, C3C und C3D sind gemergt. Der historische C3-Parent bleibt `blocked`; C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist ausschließlich `codex_ready` für `preflight_only`, `IMPLEMENTATION_AUTHORIZED=false` und nicht `done`.
...
- C3A ist über PR `#740` abgeschlossen; C3B ist über PR `#744`, C3C über PR `#748` und C3D `CREATE-GUEST-EPHEMERAL-UI-01` über PR `#751`, Implementation Head `30b160e17a44cce40afd5626b02c8e4e090da289`, mit Merge `65a89f67d1a8bb21fac41def3164270cd171b8d5` abgeschlossen. C3D-Autorisierung ist konsumiert und neue oder doppelte Dispatches sind geschlossen. C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist ausschließlich `codex_ready` für `preflight_only`; C5–C12 und G1–G5 bleiben nicht autorisiert. Der fehlgeschlagene C3-Parent-Preflight autorisiert keine Parent- oder weitere Slice-Implementierung.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) ist über PR `#734` gemergt. `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) ist über PR `#736` gemergt. Der C3-Parent bleibt nach seinem historischen Preflight nicht ausführbar; die dekomponierte Sequenz C3A–C3D ist abgeschlossen: C3A über PR `#740`, C3B über PR `#744`, C3C über PR `#748` und C3D über PR `#751`, Implementation Head `30b160e17a44cce40afd5626b02c8e4e090da289`, Merge `65a89f67d1a8bb21fac41def3164270cd171b8d5`. Für C3D gilt `IMPLEMENTATION_AUTHORIZATION_CONSUMED=true`, `NEW_IMPLEMENTATION_DISPATCH_ALLOWED=false` und `DONE=true`. C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist ausschließlich `codex_ready` mit `AUTHORIZATION=preflight_only`, `IMPLEMENTATION_AUTHORIZED=false` und `DONE=false`; C5–C12 und G1–G5 bleiben unautorisiert. Jeder spätere Slice braucht nach Abschluss aller Vorgänger einen eigenen operativen Task, einen positiven Preflight und einen frischen Branch vom dann aktuellen `main`. Die produktweite Einordnung steht in `docs/E150/EDEBATTE_MASTER_PRODUCTION_ROADMAP_2026-09-09.md`.
...
| C4 | Browser Resume, explizite Adoption und Draft Binding | `#682` | `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01`: C3A–C3D abgeschlossen; `codex_ready` ausschließlich für `preflight_only`, keine Implementierungsautorisierung |
...
C3A hat den eigenen Preflight bestanden und wurde über PR `#740` abgeschlossen; C3B über PR `#744` und C3C über PR `#748` sind `done`. C3D `CREATE-GUEST-EPHEMERAL-UI-01` ist über PR `#751`, Exact Head `30b160e17a44cce40afd5626b02c8e4e090da289`, mit Merge `65a89f67d1a8bb21fac41def3164270cd171b8d5` `done`; seine Drei-Dateien-Autorisierung ist konsumiert und geschlossen. C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist ausschließlich für den taskbezogenen Preflight `codex_ready`; `AUTHORIZATION=preflight_only`, `IMPLEMENTATION_AUTHORIZED=false`, `DONE=false`. Die folgende Source-/Owner-Evidence beschreibt den historischen Parent-Scope und ist keine pauschale Copy- oder Merge-Freigabe.
  ...
  └─ C3C Guest Claim/PII/Safety (done; PR #748; main@932d01e6a85c55e6763f070647c2d67513248e36)
       └─ C3D Guest Ephemeral UI (done; PR #751; exact head 30b160e17a44cce40afd5626b02c8e4e090da289; merge 65a89f67d1a8bb21fac41def3164270cd171b8d5; authorization consumed)
            └─ merge → refreshed main
                 └─ C4 Resume/Adoption/Draft Binding (`CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01`; codex_ready; preflight_only; implementation_authorized=false)
...
Dieses Runbook autorisiert weder den C3-Parent noch C3B, C3C oder C3D zu einer neuen oder doppelten Implementierung. C3A, C3B, C3C und C3D sind `done`; C3D wurde über PR `#751`, Implementation Head `30b160e17a44cce40afd5626b02c8e4e090da289`, mit Merge `65a89f67d1a8bb21fac41def3164270cd171b8d5` abgeschlossen, seine Autorisierung ist konsumiert und Erweiterungen sind geschlossen. C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist ausschließlich `preflight_only`; C5–C12 und G1–G5 bleiben nicht autorisiert. Dieses Runbook autorisiert kein Deployment, keine Production-DB-Arbeit, keine Secret-/Provideraktivierung und keine Änderung an `#590`, `#682`, `#724` oder `#727`. Produktweite Parallel-Lanes und Production Gates werden ausschließlich im Master-Roadmap-Dokument eingeordnet.
