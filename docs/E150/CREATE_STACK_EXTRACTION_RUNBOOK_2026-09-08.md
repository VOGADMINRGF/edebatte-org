...
Status: Governance- und Extraktionsplan; C1, C2 und C3A sind gemergt, der C3-Parent bleibt blockiert und ausschließlich C3B ist nach positivem Preflight in der belegten Grenze zur Implementierung autorisiert

Zielbasis: `main@967d567811b41a17014d01ec8d47676195351c2a` (enthält C1 über PR `#734`, C2 über PR `#736`, die Master-Governance aus PR `#738`, den C3A-Preflight aus PR `#739`, C3A über PR `#740` und die C3B-Preflight-Autorisierung aus PR `#741`)
...
- C3A ist über PR `#740` abgeschlossen; ausschließlich C3B bleibt `codex_ready` und ist nach positivem Task-, Security-, Extraktions-, Size- und Dependency-Preflight in der belegten Grenze zur Implementierung autorisiert. Die Implementierung hat noch nicht stattgefunden. C3C–C3D bleiben `blocked`, C4–C12 und G1–G5 bleiben nicht autorisiert, und der fehlgeschlagene C3-Parent-Preflight autorisiert keine weitere Implementierung.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) ist über PR `#734` gemergt. `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) ist über PR `#736`, Implementation Head `a86d50128fed9ec86a0c7884c7f5a11a978b79e1`, als `main@7896cd41ba366b3b7de5725872675170e8f3b86b` gemergt. Für `CREATE-ANONYMOUS-SESSION-ABUSE-GUEST-CLAIM-01` (C3) gilt `TASK_PREFLIGHT=PASS`, `PII_SECURITY_PREFLIGHT=FAIL`, `SIZE_GATE=FAIL_SPLIT_REQUIRED`, `SECURITY_GATE=FAIL_BLOCKED`, `WORK_REQUIRED=true` und `SAFE_TO_IMPLEMENT=false`. Der Parent bleibt nicht ausführbar. C3A ist über PR `#740` abgeschlossen; der C3B-Preflight auf `main@967d567811b41a17014d01ec8d47676195351c2a` schloss alle fünf Gates mit `PASS` und `BLOCKERS=NONE`. Ausschließlich C3B bleibt `codex_ready` und ist in der belegten Grenze zur Implementierung autorisiert; C3B ist noch nicht implementiert. C3C–C3D, C4–C12 und G1–G5 bleiben unautorisiert. Jeder spätere Slice braucht nach Abschluss aller Vorgänger einen eigenen operativen Task, einen positiven Preflight und einen frischen Branch vom dann aktuellen `main`. Die produktweite Einordnung steht in `docs/E150/EDEBATTE_MASTER_PRODUCTION_ROADMAP_2026-09-09.md`.
...
| C3 | Anonymous Session, Abuse Gates und redacted Guest Claim | `#724`, `#682` | Parent `blocked`; C3A über PR `#740` done; nur C3B `codex_ready` und nach positivem Preflight implementierungsautorisiert; C3C–C3D seriell `blocked` |
...
C3A hat den eigenen Preflight bestanden und wurde über PR `#740`, Exact Head `39a5e2567885ec1d33c74c51b553561d7bd91637`, als `main@52b1995706e0b69c01413171b63b4d7888dbb16b` gemergt. Der C3B-Preflight auf `main@967d567811b41a17014d01ec8d47676195351c2a` belegt `TASK_PREFLIGHT=PASS`, `SECURITY_PREFLIGHT=PASS`, `EXTRACTION_PREFLIGHT=PASS`, `SIZE_PREFLIGHT=PASS`, `DEPENDENCY_GATE=PASS` und `BLOCKERS=NONE`. Ausschließlich C3B bleibt `codex_ready` und ist jetzt innerhalb dieses positiven Preflight-Vertrags zur Implementierung autorisiert; die Implementierung hat noch nicht stattgefunden. C3C–C3D bleiben `blocked`, und C4 hängt weiterhin vom Abschluss aller vier Slices ab. Die folgende Source-/Owner-Evidence beschreibt den historischen Parent-Scope und ist keine pauschale Copy- oder Merge-Freigabe.

Die C3B-Implementierungsfreigabe ist auf neun Dateien begrenzt. Geändert werden dürfen ausschließlich `apps/web/src/features/create/createMutationSecurityContract.ts`, `apps/web/src/features/create/createRouteSecurity.ts`, `apps/web/src/app/api/create/session/route.ts`, `apps/web/src/app/create/CreateClient.tsx`, `apps/web/tests/create-route-security.contract.test.ts` und `apps/web/tests/create-mode.save.route.test.ts`; neu hinzukommen dürfen ausschließlich `apps/web/src/features/create/createAbuseGuard.ts`, `apps/web/tests/create-abuse-guard.contract.test.ts` und `apps/web/tests/create-antispam-client.contract.test.ts`. Die neunte Datei ist eine bestehende Testdatei und darf nur den minimalen steuerbaren Mock für den realen persistenten Limiter erhalten, dessen Default den verfügbaren und erlaubenden Limiter abbildet. Auf `main@67c3fb8b6e62201587964e0226714cf3e84e9b4c` und im sicher verwahrten aktuellen C3B-Implementierungsstand bestehen ohne diesen Mock jeweils 2/19 Tests; 17/19 scheitern als Test-Harness-Inkompatibilität, nicht als belegte Runtime-Regression. Assertions sowie Origin-, Fetch-Metadata-, CSRF-, Honeypot-, Parser- und Body-Size-Prüfungen dürfen weder abgeschwächt noch umgangen werden. In `CreateClient.tsx` ist ausschließlich der minimale initial leere, längenbegrenzte, nicht sichtbare Honeypot-State samt off-screen Control im bestehenden Composer und Weitergabe an die fünf vorhandenen `createMutationRequestHeaders()`-Callsites autorisiert; `SharedCreateComposer.tsx`, Request-Bodies und Submit-Pfade bleiben unverändert. Autorisiert sind nur Same-Origin, Fetch Metadata, CSRF, Honeypot, persistente Rate-/Risk-Limits, Cooldown/Duplicate-Handling, Content-Type, tatsächlich gemessene und vor dem Parse begrenzte Body-Größe, fail-closed Parser/Limiter, minimierte beziehungsweise gehashte Subjects, feste sichere öffentliche Fehler, C3A-Session-Verifikationskompatibilität, Leak-Schutz und fokussierte Regressionen. Weitere Runtime- oder Testdateien, sichtbare Browser-UX, Planner-/Intake-Fachsemantik, Guest Claim/Adoption, browsergenerierte anonyme Session-IDs, Session-/Local-Storage-Resume, implizites Session-Priming, Production-DB, Migration, Deployment, Provider-/Secret-Aktivierung und Auto-Publish bleiben ausgeschlossen.
...
main (C1, C2 und C3A gemergt; C3A done über PR #740)
  └─ C3B Anonymous Abuse/Route Security (codex_ready; Implementierung ausschließlich in positiver Preflight-Grenze autorisiert, noch nicht erfolgt)
...
Dieses Runbook autorisiert weder den C3-Parent noch C3C–C3D, C4–C12 oder G1–G5 zur Implementierung. C3A ist über PR `#740` gemergt; ausschließlich C3B bleibt `codex_ready` und ist in der oben festgelegten positiven Preflight-Grenze zur Implementierung autorisiert. Es autorisiert keinen Merge der Quell-PRs, kein Deployment, keine Production-DB-Arbeit, keine Secret-/Provideraktivierung und keine Änderung an `#590`, `#682`, `#724` oder `#727`. C3C–C3D benötigen nach Abschluss ihres jeweiligen Vorgängers weiterhin einen eigenen `codex_ready`-Status und positiven Preflight; die vorhandene technische Ownership von C4–C12 und G1–G5 bleibt unverändert. Produktweite Parallel-Lanes und Production Gates werden ausschließlich im Master-Roadmap-Dokument eingeordnet.
