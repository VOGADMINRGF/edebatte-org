...
- C3A–C3D sind abgeschlossen. Der C4-Parent `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` hat auf `main@c736241fe52b7c1c8d1d4c2fe4651c6159c67847` `FAIL_SPLIT_REQUIRED` ergeben und ist `blocked`, nie direkt implementierbar. C4A `CREATE-GUEST-ADOPTION-PREPARATION-FOUNDATION-01` ist ausschließlich `codex_ready` für `preflight_only`; C4B `CREATE-AUTHENTICATED-GUEST-ADOPTION-DRAFT-RESUME-01` bleibt bis C4A done `blocked` ohne Autorisierung. C5–C12 und G1–G5 bleiben nicht autorisiert.

`CREATE-PLANNER-TIMING-FOUNDATION-01` (C1) und `CREATE-WORKSPACE-MOBILE-PRESENTATION-01` (C2) sind gemergt. Der C3-Parent bleibt nach historischem Preflight nicht ausführbar; C3A–C3D sind abgeschlossen. C4-Parent `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist auf `main@c736241fe52b7c1c8d1d4c2fe4651c6159c67847` mit `FAIL_SPLIT_REQUIRED`, `SAFE_TO_IMPLEMENT=false` und `DONE=false` `blocked`. C4A ist allein für einen positiven taskbezogenen Preflight `codex_ready`; C4B bleibt `blocked` bis C4A done. C5–C12 und G1–G5 bleiben unautorisiert.
...
| C4 | Browser Resume, explizite Adoption und Draft Binding | `#682` | Parent `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` `blocked` nach `FAIL_SPLIT_REQUIRED`; C4A `CREATE-GUEST-ADOPTION-PREPARATION-FOUNDATION-01` `codex_ready` ausschließlich `preflight_only`; C4B `CREATE-AUTHENTICATED-GUEST-ADOPTION-DRAFT-RESUME-01` `blocked` bis C4A done |
                 ...
                 └─ C4 Parent (`FAIL_SPLIT_REQUIRED`; blocked; never direct implementation)
                      └─ C4A Guest Adoption Preparation (`FAIL_BLOCKED`; `AT_REST_PROTECTION`; implementation unauthorized)
                           └─ Temporary Payload At-Rest Protection (`codex_ready`; `preflight_only`)
                                └─ foundation done → merge → refreshed main → fresh C4A preflight
                                     └─ C4A PASS → separate authorization → implementation → merge → refreshed main
