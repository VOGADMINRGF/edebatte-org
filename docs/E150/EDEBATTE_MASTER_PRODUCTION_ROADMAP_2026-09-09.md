...
Basis: `main@7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d`

Status: Governance- und Architekturmanifest; keine Runtime-, Schema-, Provider-, Secret-, Deploy-, Publish- oder Produktionsfreigabe

Aktueller Create-Governance-Sync 2026-09-13: C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`; C4A1 `CREATE-GUEST-ADOPTION-PREPARATION-SERVER-FOUNDATION-01` ist über PR `#762`, Implementation Head `d503e97848dbfb8726b2a3a930748f9667b984c8`, Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` `done`. Final gilt `NO_PREPARATION_BROWSER_CARRIER_V1`; die verifizierte C3A-Session bleibt die alleinige Browser-Transition-Bindung. Ausschließlich C4B `CREATE-AUTHENTICATED-GUEST-ADOPTION-DRAFT-RESUME-01` ist für einen frischen Preflight `codex_ready` mit `AUTHORIZATION=preflight_only`; `IMPLEMENTATION_AUTHORIZED=false`. C4C bleibt `blocked`; C5–C12 bleiben nicht autorisiert.
...
G2–G5 behalten ihre vorhandenen Owner und Abhängigkeiten; G4 bleibt der einzige Public-QR-Guard-Owner. Unabhängige Guard- und Create-Arbeit wird nicht künstlich vollständig serialisiert. C9 folgt weiterhin erst nach C6–C8 und G1. C4-Parent und C4A-Dekompositions-Parent bleiben `blocked`; C4A1 ist `done`; ausschließlich C4B ist für frischen `preflight_only` `codex_ready`. C4B-Implementation, C4C, C5–C12 und G1–G5 werden durch dieses Manifest nicht implementierungsautorisiert.
...
Der C4-Parent bleibt `blocked`. Die At-Rest-Foundation ist abgeschlossen; C4A bleibt nach `FAIL_SPLIT_REQUIRED` ein `blocked` Dekompositions-Parent. C4A1s historisches `FAIL_BLOCKED` wegen `DURABLE_FAILED_REPREPARE_REVOCATION` bleibt Evidence, ist aber aufgelöst: die Sechs-Dateien-Server-Foundation mit `DURABLE_REPREPARE_REVOCATION_BARRIER_V1` wurde über PR `#762`, Implementation Head `d503e97848dbfb8726b2a3a930748f9667b984c8`, als Merge `7bb2488ffd26a94a0ba35ee2e5ef84ae53c9c23d` abgeschlossen. Final gilt `NO_PREPARATION_BROWSER_CARRIER_V1`: C4A1 führt weder Preparation-ID-Cookie noch Browser-/URL-/Storage-Locator; die verifizierte C3A-Session ist die einzige Transition-Bindung und C4B lädt den aktuellen prepared Slot binding-only. Ausschließlich ein frischer C4B-Preflight ist autorisiert; C4B-Implementation und C4C sind nicht autorisiert. C4C folgt erst nach C4B done. C5–C12 und G1–G5 bleiben nicht autorisiert.
