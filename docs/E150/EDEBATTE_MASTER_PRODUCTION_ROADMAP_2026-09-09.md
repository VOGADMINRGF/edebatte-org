...
| C3B | `CREATE-ANONYMOUS-ABUSE-ROUTE-SECURITY-01` | Same-Origin/Fetch Metadata/CSRF, Honeypot, persistente Actor-/IP-/Session-/Client-Limits, Cooldown, expliziter Content-Type, vor dem Parse gemessene Body-Grenze, malformed/unknown/unsupported fail-closed, unavailable limiter fail-closed | Planner-/Intake-Fachsemantik, Claim/Single Flight/PII-URL-Analyse, Browser-UI, Adoption und Account Draft | `codex_ready`; `implementation_authorized`; Implementierung noch nicht erfolgt und ausschließlich in der positiven Preflight-Grenze zulässig |
