...
Der C4-Parent bleibt `blocked`. Die At-Rest-Foundation ist abgeschlossen; C4A bleibt nach `FAIL_SPLIT_REQUIRED` ein Dekompositions-Parent. C4A1s historisches `FAIL_BLOCKED` wegen `DURABLE_FAILED_REPREPARE_REVOCATION` bleibt Evidence. Seine Sechs-Dateien-Server-Foundation mit `DURABLE_REPREPARE_REVOCATION_BARRIER_V1` ist ausschließlich als Draft-PR `#762` aktiv; ihr neuer Dispatch ist konsumiert. Die P1-Review-Korrektur `NO_PREPARATION_BROWSER_CARRIER_V1` verlangt, dass C4A1 weder Preparation-ID-Cookies noch sonstige Browser-Locators führt: die verifizierte C3A-Session ist die einzige Transition-Bindung, und das spätere C4B lädt den einen prepared Slot binding-only. Keine C4B-/C4C- oder Production-Freigabe. C4B folgt erst nach C4A1 done, C4C erst nach C4B done. C5–C12 und G1–G5 bleiben nicht autorisiert.
