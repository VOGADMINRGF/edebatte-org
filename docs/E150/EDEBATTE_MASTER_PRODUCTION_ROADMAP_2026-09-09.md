...
C4 `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist `codex_ready` ausschließlich für einen taskbezogenen Preflight; `AUTHORIZATION=preflight_only`, `IMPLEMENTATION_AUTHORIZED=false` und `DONE=false`. Der Preflight muss die Anonymous→Auth-Transition, Rotation oder sichere Rebinding-Grenze, serverautoritatives Account-Identity, fail-closed Cross-Account-Adoption, explizite idempotente Adoption mit Receipt und Replay-Schutz sowie genau einen Account-Draft prüfen. Die anonyme Session ist kein Auth-Credential, es gibt keine stille Login-Adoption und keine Rohdaten in URLs, Logs oder Telemetrie. C5–C12 und G1–G5 bleiben nicht autorisiert.
