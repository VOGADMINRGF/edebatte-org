...
Basis: `main@91ac96aa8dc12ab38e4912b6e4d29807f124cde4`
...
| C3C | `CREATE-GUEST-CLAIM-PII-SAFETY-01` | Preflight für die Implementierungsgrenze von server-authoritativem Guest Claim/PII/Single Flight, ausschließlich servergenerierter Correlation, rekursiver PII-/Secret-Erkennung, Signed-URL-Leakage-Erkennung und allowlisteten sicheren Fehlern | Adoption, Resume, Account Draft, Browserpersistenz, Production-Aktivierung sowie jede Runtime-Implementierung | `codex_ready`; `preflight_only`; `implementation_authorized=false`; Task-, Security-, Extraktions- und Size-Preflight vor jeder Implementierungsfreigabe |
...
Der C3C-Preflight muss belegen, dass beide Allowlist-Schemas rohen Guest-Text, Source-Input, sensitive oder signierte URLs, Planner-Prompt/-Trace, rohe Providerantwort, Token/Credential/Secret, beliebige Fehlerstrings und beliebige Metadatenblobs ausschließen. Rekursive PII-/Secret-/Signed-URL-Leakage-Erkennung ist vor Persistenz und separat vor API-Return zu prüfen; jeder Fund muss ohne unsichere Persistenz und ohne unsicheren Response stoppen. Human-readable Diagnostics dürfen ausschließlich aus festen servereigenen Failure-Codes abgeleitet werden. Eine Client-Correlation darf niemals Claim-Identität, Single-Flight-Key, Replay-Namespace oder freie persistierte Metadaten beeinflussen; die authoritative Correlation ist servergeneriert. Der Preflight darf keine Adoption-, Resume-, Account-Draft-, Browserpersistenz- oder Production-Semantik autorisieren.
