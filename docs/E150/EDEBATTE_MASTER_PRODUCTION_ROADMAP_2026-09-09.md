...
> **Historisches Status-Update nach PR #748:** Die unmittelbar vorstehende C3B-Preflight-Erzählung beschreibt den Stand vor der C3C-Implementierung. C3C ist über PR `#748` `done`; zum Zeitpunkt dieses historischen Updates war C3D nur `preflight_only` und `IMPLEMENTATION_AUTHORIZED=false`. Der aktuelle C3D-Status ist `done`; C4-Parent ist `blocked` nach `FAIL_SPLIT_REQUIRED`; ausschließlich C4A ist `preflight_only`, C4B ist `blocked`.
...
Der C4-Parent `CREATE-GUEST-RESUME-ADOPTION-DRAFT-BINDING-01` ist nach dem revisionsgebundenen Preflight auf `main@c736241fe52b7c1c8d1d4c2fe4651c6159c67847` `blocked` mit `FAIL_SPLIT_REQUIRED`, `SAFE_TO_IMPLEMENT=false` und `DONE=false`. C3C persistiert keinen Rohtext, C3D bleibt ephemer, Login rotiert oder bindet die anonyme Session nicht; Payload-/Receipt-/Auth-Transition sind getrennte Sicherheitsdomänen. C4A `CREATE-GUEST-ADOPTION-PREPARATION-FOUNDATION-01` ist nach seinem Preflight auf `main@0103c90e0364f3da2f86ab56ac4a230fafc74049` `blocked` mit `PREFLIGHT_RESULT=FAIL_BLOCKED`, `BLOCKER=AT_REST_PROTECTION`, `IMPLEMENTATION_AUTHORIZED=false`, `DONE=false` und `SPLIT_REQUIRED=false`: Der temporäre Rohpayload muss die Login-Navigation überdauern, doch eine anwendungsseitige At-Rest-Schutz-Primitive samt genehmigtem Key-Loading-/Rotation-Contract fehlt. TTL und C3C-Safety sind nicht äquivalent. Ausschließlich `CREATE-TEMP-PAYLOAD-AT-REST-PROTECTION-01` ist `codex_ready` für `AUTHORIZATION=preflight_only`; dessen möglicher Abschluss autorisiert C4A nicht direkt, sondern erfordert einen frischen C4A-Preflight auf refreshed main. C4B `CREATE-AUTHENTICATED-GUEST-ADOPTION-DRAFT-RESUME-01` bleibt bis C4A done `blocked` und `AUTHORIZATION=none`. C5–C12 und G1–G5 bleiben nicht autorisiert.
