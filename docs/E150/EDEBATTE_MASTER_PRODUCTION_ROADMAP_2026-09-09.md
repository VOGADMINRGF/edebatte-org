...
> **Status-Update nach PR #748:** Die unmittelbar vorstehende C3B-Preflight-Erzählung beschreibt den historischen Stand vor der C3C-Implementierung. C3C ist über PR `#748` `done`; ausschließlich C3D ist `codex_ready` für `preflight_only`, mit `IMPLEMENTATION_AUTHORIZED=false` und weiterhin nicht `done`.
