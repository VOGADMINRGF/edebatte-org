...
Basis: `main@7b5193f6ad26befb925f1187f9f0664fa30e603b`
...
Golden A + Golden B + production provider/feed gates → T8
...
| --- | --- | --- | --- | --- |
| C3A | `CREATE-ANONYMOUS-SESSION-FOUNDATION-01` | Signierte anonyme Session: HMAC/signiertes HttpOnly-SameSite-Cookie, serverseitige UUID-/Expiry-Prüfung, `POST /api/create/session`, Issuance-Rate-Limit, awaited Initialisierung, minimale wahrheitsgemäße Response | Intake, Planner, Single Flight, Account Draft, Adoption, Resume, Source Fetch, roher Browser-Snapshot | `blocked`; Architektur-/Security-Review des Splits |
| C3B | `CREATE-ANONYMOUS-ABUSE-ROUTE-SECURITY-01` | Same-Origin/Fetch Metadata/CSRF, Honeypot, persistente Actor-/IP-/Session-/Client-Limits, Cooldown, expliziter Content-Type, gemessene Body-Grenze, malformed/unknown/unsupported fail-closed, unavailable limiter fail-closed | Planner-/Intake-Fachsemantik jenseits der Security-Grenze | `blocked`; C3A plus Architektur-/Security-Review |
| C3C | `CREATE-GUEST-CLAIM-PII-SINGLE-FLIGHT-01` | Server-authoritative Intake-Claim; Session/Actor/Input-Hash/Correlation; servergenerierte/-normalisierte Correlation; begrenztes rekursives Decoding; PII-/Secret-Prüfung in Userinfo, Path, Query-Key/-Value und Fragment; Token- und Signed-Resource-URLs; non-retaining `source_pending`; allowlisted Failure-Codes; Single Flight/Replay/Lease Recovery | Redigierte oder semantisch veränderte sensitive URL fetchen; C4-Adoption-Felder | `blocked`; C3B plus Architektur-/Security-Review |
...
C4 hängt nun hart vom Abschluss aller vier Slices ab. Seine technische Ownership sowie die von C5–C12 und G1–G5 bleibt unverändert.
...
| --- | --- | --- | --- |
| T0 | `DECISION-DOSSIER-ARCHITECTURE-CONTRACT-01` | Contract-Mapping für SystemQuestion, ResearchObject, ContextEvent, Metric/Projection/Assumption, ScenarioSet/Scenario, Impact/Tradeoff, Comparator und DecisionBinding; keine Migration | Standard angenommen; Architekturreview |
...
| T2 | `DOSSIER-RESEARCH-WORKSPACE-01` | Dossiergebundener ResearchPlan mit WorkItems, Fragen, Source Gaps, Artefakten, Runs/Checkpoints, Completeness, Freshness, Retry/Recovery, Human Gates, Budget/Cost; keine zweite Dossier-ID | T1 |
| T3 | `DOSSIER-CONTEXT-METRICS-EPISTEMIC-01` | Historische-/Kontext-/Reformereignisse, Current-System-Modell, Metriken/Serien, Projektionen/Modelle, Annahmen, Epistemik, Source Relations, stale/superseded | T2 |
...
| T5 | `DOSSIER-SCENARIO-IMPACT-COMPARATOR-01` | Impact, Trade-offs, Gruppen, Verteilungs-/Generationseffekte, Kosten/Finanzierung, Recht, internationale Comparatoren, Transferierbarkeit, Unsicherheitsbandbreiten und Stresstests | T4 |
...
| T8 | `SELF-FEEDING-TOPIC-RESEARCH-ORCHESTRATION-01` | Signal → Qualification Candidate → Research Mission → validierte strukturierte Schritte → Scenario-/Impact-Kandidaten → Human Gates → publish-ready Decision-Dossier-Draft; keine Auto-Truth, Veröffentlichung oder Entscheidungsaktivierung | T7, beide Golden Cases sowie Production-Provider-/Feed-Gates |

### Decision Readiness

Das Gate aus dem System-Question-Standard ist verbindlich. `decision_ready` bleibt false, wenn eine materielle Dimension fehlt oder nur durch erfundene Prosa/Zahlen gefüllt werden könnte. Jede Dimension muss ausreichend belegt/reviewed oder ausdrücklich als ungelöste materielle Evidenzlücke ausgewiesen sein; eine solche Lücke blockiert die Aktivierung. Evidenzpriorität, Widersprüche, Frische, Jurisdiktion, Methodik und Interessenkonflikte bleiben sichtbar.
...
T8 bleibt auf beide Fälle und Production-Provider-/Feed-Gates blockiert. Dadurch wird keine Architektur aus nur einer Policy-Domäne verallgemeinert.

## 5. Alpha2 Control Plane

Alpha2 erweitert die bestehende Control Plane und ist kein zweiter Orchestrator. Die Konvergenzordnung ist:
...
| UX | Mobile; Tastatur; Screenreader; Empty/Partial/Error; Lokalisierung; RTL, wo anwendbar |
| Operations | Observability; actionable errors; Cost-/Rate-Limits; Kill Switch, wo relevant; Rollback |
| Performance | begrenzte Latenz, Payload und Ressourcennutzung |
