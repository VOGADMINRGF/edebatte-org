...
Basis: `main@e4b2af9e7f269370f0a4f9f5bc851adbfe986d04`
...
Die C3A-Autorisierung gilt ausschließlich für den eigenen Preflight. Dieser muss vor Code belegen: eine servergenerierte, opake, unvorhersagbare und manipulationssichere kanonische Session-Identität ohne clientseitige Autorität; ein `HttpOnly`-/`SameSite`-Cookie mit umgebungs- und produktionskonformem `Secure`, begrenzter Laufzeit sowie korrekter Path-/Domain-Policy; serverseitige Signatur- und Expiry-Validierung; fail-closed Verhalten für abgelaufene, manipulierte oder malformed Cookies; begrenzte Session-Issuance samt definiertem Fehlerfall und mit C3B kompatibler Abuse-Grenze; sowie awaited Initialisierung und eine minimale wahrheitsgemäße Response von `POST /api/create/session`. Token und Cookie enthalten weder rohe Guest-/Source-Inhalte noch PII oder Secrets; Session-Secrets und rohe Identifikatoren gelangen nicht in unsichere Logs, Analytics, Traces oder Error Payloads. TTL, Expiry-Semantik, Cleanup Owner, stale-/Orphan-Cleanup, sicheres Restart-Verhalten und die Vermeidung unbegrenzt gültiger anonymer Credentials sind explizite Lifecycle-Gates.

Der C3A-Preflight muss Source-Hunks, exakte Dateiliste, Collision State, API-Grenze, Cookie-/Session-Contract, Issuance-Abuse-Grenze, TTL/Lifecycle, Tests, Size Gate, Security Gate und die C1-/C2-Regressionen festlegen. C3A übernimmt weder anonyme Intake-Semantik, Guest Claim, Single Flight, Planner, Source-/Linkanalyse, rekursive PII-URL-Behandlung, persistierte Guest Results, Browser Resume, Adoption, Account-Draft-Bindung, Login-/Session-Fixation-Transitionen noch Jurisdiction, Topic Matching, Notifications, Progress/SSE, Public Guards, Voxy, QR oder Publishing. C3B–C3D bleiben `blocked`; C4 bleibt vom Abschluss aller vier Slices abhängig.
