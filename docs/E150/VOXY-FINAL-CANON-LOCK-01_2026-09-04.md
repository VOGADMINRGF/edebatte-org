...
- Der offizielle lokale Renderbefehl bindet den tatsächlich ausgecheckten `HEAD` selbst an `VOXY_HOMEPAGE_REFERENCE_FILMS_COMMIT_SHA`; ein ausdrücklich gesetzter abweichender SHA schlägt fail-closed fehl. Damit bleibt die Exact-Head-Prüfung erhalten, ohne dass Betreiber den SHA manuell kopieren müssen.
