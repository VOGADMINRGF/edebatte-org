...
| --- | --- | --- |
| `apps/web/src/features/create/createMutationSecurityContract.ts` — `CREATE_MUTATION_*`, `createMutationRequestHeaders` | Honeypot- und optionaler Client-Signal-Header sowie Hidden-Trap-Propagation aus `3011d46a` | Hunkgenau auf den aktuellen Contract übertragen; bestehende CSRF-/Content-Type-Header erhalten. `createClientSessionId`, `readOrCreateClientSessionId`, SessionStorage und `primeCreateSecuritySession` ausdrücklich nicht übernehmen. |
...
1. `apps/web/src/features/create/createMutationSecurityContract.ts`
   ...
   - autorisiert: begrenztes Honeypot-Signal, begrenztes optionales Client-Signal, gemeinsames reines Origin-/Fetch-Metadata-/CSRF-Predicate, Hidden-Trap-Propagation
...
4. `apps/web/tests/create-route-security.contract.test.ts`
   - ausschließlich um die C3B-Transport-, Session-, Limiter-, Leakage- und Failure-Verträge erweitern

### Neu

5. `apps/web/src/features/create/createAbuseGuard.ts`
6. `apps/web/tests/create-abuse-guard.contract.test.ts`
7. `apps/web/tests/create-antispam-client.contract.test.ts`
...
| `HONEYPOT_GATE` | `PASS` | Ein begrenzter Hidden-Trap kann vor Body-Verarbeitung abgewiesen werden, ohne sichtbare UI auszuweiten. |
...
```text
EXPECTED_FILES_CHANGED=7
EXPECTED_NEW_FILES=3
EXPECTED_RUNTIME_LOC_DELTA=+260–340
EXPECTED_TEST_LOC_DELTA=+260–360
SIZE_GATE=PASS
```

Der Slice bleibt damit schmal, security-fokussiert und unabhängig reviewbar. Eine Überschreitung der Sieben-Dateien-Grenze oder zusätzliche Runtime-Datei stoppt die Implementierung bis zu einer neuen Collision-/Preflight-Entscheidung.
